...
class TestDisaggregatedInferenceIsRejectedLoudly:
    """DI must fail with an explanation, on the path operators actually use.

    Two distinct entry points, because they fire at different times:

    * ``kv_connector_module_path`` makes vLLM import the connector module during
      ``VllmConfig`` construction -- before any platform hook runs. That path is
      covered by the import-time guard in ``neuron_nixl_connector``.
    * Without a module path, the platform's ``check_and_update_config`` guard is
      what rejects it.

    The first is the realistic invocation, and it is the one a config-time-only
    guard would miss entirely.
    """

    def test_importing_the_nixl_connector_raises_an_explained_error(self):
        from vllm_neuron.vllm.di_support import UnsupportedDIConfigError

        with pytest.raises(UnsupportedDIConfigError) as excinfo:
            import vllm_neuron.vllm.kv_connector.neuron_nixl_connector  # noqa: F401

        message = str(excinfo.value)
        assert "NixlConnectorWorker" in message
        assert "--kv-transfer-config" in message

    def test_import_error_is_preserved_as_the_cause(self):
        """Keep the original ImportError chained, for debugging the port later.

        The friendly message is for operators; whoever restores the connector
        needs the actual missing symbol, which ``raise ... from exc`` preserves.
        """
        from vllm_neuron.vllm.di_support import UnsupportedDIConfigError
    ...
    def test_the_decode_bench_connector_still_imports(self):
        """Scope guard: only NIXL is broken, and DI must not be blanket-disabled."""
