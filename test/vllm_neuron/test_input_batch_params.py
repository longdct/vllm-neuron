...
Covers the P0.3 break: ``InputBatch`` has no ``pin_memory`` parameter -- it
reads the module-level ``PIN_MEMORY`` itself -- and takes per-group
``max_num_blocks_per_req``. That kwarg defaults to ``None``, in which case the
block table derives row lengths from a single block size, which is wrong the
moment the groups are heterogeneous; so passing it explicitly is the point.

``slot_mapping_modes`` is deliberately not covered: it arrives in vLLM 0.26 and
0.24's ``InputBatch`` does not accept it.
...
def vllm_config():
    ...
    The derivation no longer reads ``vllm_config`` -- 0.24 takes the CP world
    size from the live process groups -- but it stays in the signature, so the
    fixture stays too. Nothing here loads a model, so ``ModelConfig`` is skipped
    entirely.
    """
    return VllmConfig(
