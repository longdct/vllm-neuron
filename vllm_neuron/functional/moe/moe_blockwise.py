...
def build_blockwise_mapping(
    ...
    Args:
        ...
        tp_degree: number of ranks that shard each expert's intermediate
            dimension (tp_degree = world_size / ep_degree). When tp_degree > 1,
            expert work is column-sharded across ranks and collectives
            (all_gather, all_reduce) combine partial results. When tp_degree = 1
            (full EP), each rank processes all its local experts independently
            with no collectives. Defaults to 1.
        padding_mask: [T,] boolean mask where True = real token, False = padding.
            If not specified, all tokens are treated as real.
        rank: scalar tensor containing the global rank index, used to derive
            the column shard position (rank % tp_degree) at runtime. Passing
            rank as a tensor avoids baking it as a constant in the FX graph,
            which would cause per-rank recompilation. If None, falls back to
            moe_group.rank_in_group (static, causes unique graphs per rank).
...
def _build_blockwise_mapping_kernel(
    ...
    Args:
        ...
        rank: optional runtime rank tensor (see build_blockwise_mapping)
    ...
    if sharded:
        ...
        if rank is not None:
            ...
            col_start_id = (
                (rank_to_col(rank, tp_degree, world_size) * E_kernel)
                .view(1)
...
def _build_blockwise_mapping_torch(
    ...
    if E_local % tp_degree == 0 and tp_degree > 1:
        ...
        if rank is not None:
            world_size = get_world_group().world_size
            col_start = rank_to_col(rank, tp_degree, world_size) * E_shard
            expert_mask_shard = torch.narrow(expert_mask, 1, col_start, E_shard)
        else:
            col_start_int = moe_group.rank_in_group * E_shard
            expert_mask_shard = expert_mask[:, col_start_int : col_start_int + E_shard]
        tokens_per_expert_local = torch.sum(expert_mask_shard, dim=0)
        tokens_per_expert = moe_group.all_gather(tokens_per_expert_local, dim=0)
    else:
    ...
    if E_local % tp_degree == 0 and tp_degree > 1:
        ...
        if rank is not None:
            world_size = get_world_group().world_size
            col_start = rank_to_col(rank, tp_degree, world_size) * E_shard
            local_positions = torch.narrow(
                token_position_by_id_and_expert, 1, col_start, E_shard
            )
            local_mask = torch.narrow(expert_mask, 1, col_start, E_shard)
        else:
            col_start_int = moe_group.rank_in_group * E_shard
            local_positions = token_position_by_id_and_expert[
                :, col_start_int : col_start_int + E_shard
            ]
            local_mask = expert_mask[:, col_start_int : col_start_int + E_shard]
