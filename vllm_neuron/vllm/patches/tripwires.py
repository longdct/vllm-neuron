...
@register(Phase.PLATFORM_CONFIG, "tripwire:parallel_config_all2all_literal")
def _check_parallel_config_all2all_literal() -> None:
    """``ParallelConfig.all2all_backend`` must still be a locatable Literal.

    The registration rewrites that Literal to admit ``"neuron"``. If pydantic
    re-nests model schemas the old fixed-depth walk either raises or edits the
    wrong node, and the Neuron all2all backend is rejected at config validation.

    This only proves the field is *findable*. That the edit takes effect is
    checked by the registration itself (which self-verifies) and by
    ``test_all2all_backend_registration_takes_effect``.
    """
    from vllm.config import ParallelConfig

    from vllm_neuron.vllm.patches.guards import PatchError, find_literal_field_schema

    schema = require_attr(
        ParallelConfig, "__pydantic_core_schema__", patch="all2all_backend_literal"
    )
    if find_literal_field_schema(schema, "all2all_backend") is None:
        raise PatchError(
            "ParallelConfig.all2all_backend is no longer a Literal field reachable "
            "in __pydantic_core_schema__; the 'neuron' all2all backend cannot be "
            "registered"
        )
