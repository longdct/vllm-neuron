...
| 512-d direct NKI device execution | **P2.d sub-gate** | **done on Trn2** — synthetic decode and causal-prefill at `head_dim=512` execute through NKI's standalone path and match an FP32 oracle; reproducible runner stores inputs, outputs, versions, inventory, and latency. This does not close vLLM graph execution or the full P2.d gate |
...
| Tiny structural direct device execution | **P6 T3 sub-gate** | **one-token diagnostic done on Trn2** — every structural layer executes through eager Torch-XLA and matches CPU within `6.68e-06` max absolute error after making routed top-k portable across CPU/XLA return types; graph capture, scheduler cache I/O, and the full P6 matrix remain |
...
| Local regression suite | **T0** | **323 passed, 6 skipped** on the fresh Trn2 host; explicit NKI simulator gate remains **2 passed** |
| Remaining gates | **P2.c/P2.d/P6–P9** | **partially hardware-blocked** — direct P2 kernel numerics now pass on Trn2; graph capture still needs a Torch-2.11-compatible Neuron SDK, while integrated tiny-model execution, memory calibration, full-checkpoint scaling, and native FP8/FP4 remain |
...
| **T2** CPU compilation | **SDK-blocked in this venv** | NKI backend configs compile and the relocated `neuronx-cc` launcher is handled, but FX→HLO imports require `torch_neuronx`; AWS's available 2.9 package pins Torch 2.9 while vLLM 0.26 pins Torch 2.11 |
